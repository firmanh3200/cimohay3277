...
st.title('Produk Domestik Regional Bruto (PDRB) :blue[Kota Cimahi]')
...
with st.container(border=True):
    st.success('Tren PDRB Kota Cimahi (Milyar Rupiah)')
...
with tab1a:
    with st.container(border=True):
        st.success('Tren PDRB Berlaku menurut Lapangan Usaha Kota Cimahi (Milyar Rupiah)')
...
with tab1b:
    with st.container(border=True):
        st.success('Tren PDRB Konstan menurut Lapangan Usaha Kota Cimahi (Milyar Rupiah)')
...
with tab2a:
    with st.container(border=True):
        st.success('Tren PDRB Berlaku menurut Pengeluaran Kota Cimahi (Milyar Rupiah)')
...
with tab2b:
    with st.container(border=True):
        st.success('Tren PDRB Konstan menurut Pengeluaran Kota Cimahi (Milyar Rupiah)')
...
with st.expander('Lihat Tabel Lengkap PDRB menurut Lapangan Usaha'):
    st.warning('Produk Domestik Regional Bruto menurut Lapangan Usaha Kota Cimahi')
    df = data.sort_values(by='Tahun', ascending=False)
    st.dataframe(df, hide_index=True, use_container_width=True)
    
with st.expander('Lihat Tabel Lengkap PDRB menurut Pengeluaran'):
    st.warning('Produk Domestik Regional Bruto menurut Pengeluaran Kota Cimahi')
